...
cgenQQ <- function(data, censored, plot = TRUE, main = "Generalised QQ-plot", ...) {
  ...
  #   gamma <- numeric(n)
  #   gamma[K] <- (cumsum(log(X[n-K+1]))- K*log(X[n-K])) / cumsum(delta[n-K+1]) 
  #   # Problems with division by 0
  #   indNA <- K[cumsum(delta[n-K+1])==0]
  #   gamma[indNA] <- NA
  
  # Ordinary Hill estimates
  gamma <- numeric(n)
  gamma[K] <- cumsum(log(X[n-K+1])) / K - log(X[n-K])
  
  
  # calculate theoretical and empirical quantiles
  K <- 1:(n-1)
  
  # -log of Kaplan-Meier estimator for the survival function in X_[n-K+1]
  gqq.the[K] <- -log(1-KaplanMeier(X[n-K+1], data = X, censored = censored[sortix]) )
...
# Pareto QQplot for interval censored data
# censored is a vector which is 1 if a data point is censored and 0 otherwise,
# giving censored=0 indicates that non of the data points is censored
...
# Exponential QQplot for interval censored data
# censored is a vector which is 1 if a data point is censored and 0 otherwise,
# giving censored=0 indicates that non of the data points is censored
...
# Log-normal QQplott for interval censored data
# censored is a vector which is 1 if a data point is censored and 0 otherwise,
# giving censored=0 indicates that non of the data points is censored
