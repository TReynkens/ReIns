...
We split this in several sections: *[datasets](#datasets), [graphical methods](#graphical-methods), [estimators for the extreme value index](#estimators-for-the-evi), [estimators of quantiles and return periods](#estimators-of-quantiles-and-return-periods), [splicing](#splicing)* and *[risk measures](#risk-measures)*.
...
All three estimators (we exclude the GPD estimator again) suggest a 99.5\% VaR value around 6 000 000 NOK.
```{r, fig.width=5, fig.height=3.5, fig.align='center'}

p <- 0.05

# Estimates for 99.5% VaR
Quant(size, gamma=H$gamma, p=p, plot=TRUE, col="blue", ylim=c(0,10^4))



# Estimates for 99.5% VaR
QuantGH(size, gamma=GH$gamma, p=p, plot=TRUE, col="blue", ylim=c(0,10^4))
...
1. `SpliceFitHill`: splicing of ME and Pareto distribution(s), possibly adapted for (upper) truncation.  
...
Using `SpliceFitHill` we can fit this splicing distribution. This can take some time so we create a `SpliceFit` object using the obtained parameters to use in the remainder. A `summary` method is available for these objects which summarises the splicing distribution.

```{r, eval=FALSE}

# Splicing of Mixed Erlang (ME) and 2 Pareto pieces
# Use 3 as initial value for M
spliceFit <- SpliceFitHill(size, const=c(0.5,0.996), M=3)
```
...
To estimate the premiums of an excess-loss insurance (with retention), one can use `ExcessSplice`. It is also possible to add a limit (e.g. $L=2*M$).
```{r, fig.width=5, fig.height=3.5, fig.align='center'}
...
# Plot VaR
plot(p, VaR, xlab="p", ylab="VaR", type="l")
```
